# chroma_service.py - Load feature vectors into ChromaDB collection in batches
...
def load_embeddings_into_chroma():
    ...
    # Prepare full IDs and metadata
    ids = [f"img_{i}" for i in range(len(features))]
    embeddings = [features[i].tolist() for i in range(len(features))]
    metadatas = [{"url": str(image_paths[i]), "label": get_label_from_path(str(image_paths[i]))} for i in range(len(features))]
    ...
    print(f" Successfully inserted {len(features)} vectors into ChromaDB.")
